...
MoE expert tiering for [llama.cpp](https://github.com/ggerganov/llama.cpp). This repo is a
fork of llama.cpp with Lamio integration built in.
...
# or set LAMIO_MODEL to an absolute path
bash start.sh
```
...
`build.sh` / `build.bat` detects Git, CMake, compiler and CUDA, builds only what is missing.
...
`GET /lamio/tier-stats` returns hit/miss/eviction counters and budget usage.
